...
指定したPull Requestの未解決レビュースレッドの一覧を取得します。カーソルベースのページネーションに対応しています。

**パラメータ:**
- `owner`: リポジトリオーナー名
- `repo`: リポジトリ名
- `pullRequestNumber`: Pull Request番号
- `limit` *(省略可)*: 1回の取得件数（1〜100、デフォルト: 5）
- `after` *(省略可)*: ページネーション用カーソル（前回レスポンスの `pageInfo.endCursor`）

**返り値:**
- `count`: このページに含まれる**未解決**スレッドの件数（全ページの合計ではない）。
- `threads`: このページ内の未解決スレッド一覧。
- `pageInfo.hasNextPage`: まだ取得していない**総スレッド**（解決済み含む）が存在する場合は `true`。未解決のみを集めたい場合は、`hasNextPage` が `false` になるまで `after` を使ってページネーションを続けてください（現在ページで `threads` が0件でも、次ページに未解決がある場合があります）。
